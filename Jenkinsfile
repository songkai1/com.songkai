            ...
            parallel {
                stage('test-deploy') {
                    when {
                        ...
                        equals expected: '10.3.22.43', actual: params.toServer
                    }
                    steps {
                        sh "echo copy ie-${params.buildMoudle}.jar to 10.3.22.43 ..."
                        sh "scp ./ie-${params.buildMoudle}/target/ie-${params.buildMoudle}.jar root@10.3.22.43:/JavaWeb/gradletest.wltest.com/${params.buildMoudle}_temp.jar"
                        sh "chmod +x ./deploy.sh"
                        sh "ssh root@10.3.22.43 bash -s < ./deploy.sh deploy ${params.buildMoudle} ${params.profile} /JavaWeb/gradletest.wltest.com"
                    }
                }
            }
        ...
        stage('other') {
            ...
            parallel {
                stage('test-other') {
                    ...
                    steps {
                        sh "echo ${params.operateType} project on 10.3.22.43 ..."
                        sh "chmod +x ./deploy.sh"
                        sh "ssh root@10.3.22.43 bash -s < ./deploy.sh ${params.operateType} ${params.buildMoudle} ${params.profile} /JavaWeb/gradletest.wltest.com"
                    }
                }
            }
        }
